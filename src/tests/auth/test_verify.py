...
async def _run_verify_test(mocker, response_callback):
    mocked_client = MockAuthClient()
    mocker.patch("app.auth.routes.grpc_clients", {"auth": mocked_client})

    mocked_client.stub.VerifyUser = mocker.AsyncMock(side_effect=response_callback)

    request_data = {"token": "verify-token"}
    with TestClient(app=app) as client:
        response = client.post("/auth/verify", json=request_data)

    expected_request = auth_pb2.Token(**request_data)
    mocked_client.stub.VerifyUser.assert_called_once_with(expected_request)

    return response


@pytest.mark.asyncio
async def test_verify_mocked_success(mocker):
    ...
    response = await _run_verify_test(mocker, lambda *_: grpc_response)
...
@pytest.mark.asyncio
async def test_verify_mocked_error(mocker):
    ...
    response = await _run_verify_test(mocker, create_rpc_error)
